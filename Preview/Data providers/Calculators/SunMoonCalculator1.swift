...
class SunMoonCalculator1 {
	...
	static func getDate(jd:Double) throws -> Date {
		...
		let h:Double = ((exactDay - Double(day)) * SunMoonCalculator1.SECONDS_PER_DAY) / 3600,
				hour:Int = Int(h),
	...
	static func getMoonPhaseName(lunarAge:Double) -> MoonPhase? {
	...
	private func setUTDate(_ jd:Double) {
		self.jd_UT = jd
		self.t = (jd + TTminusUT / SunMoonCalculator1.SECONDS_PER_DAY - SunMoonCalculator1.J2000) / SunMoonCalculator1.JULIAN_DAYS_PER_CENTURY
	}
	...
	private func getSun() -> [Double] {
		...
		return [slongitude, slatitude, sdistance, atan(696000 / (SunMoonCalculator1.AU * sdistance))]
	}

	private func getMoon() -> [Double] {
		// MOON PARAMETERS (Formulae from "Calendrical Calculations")
		let phase:Double = SunMoonCalculator1.normalizeRadians((297.8502042 + 445267.1115168 * t - 0.00163 * t * t + t * t * t / 538841 - t * t * t * t / 65194000).toRadians)
		...
		moonAge = SunMoonCalculator1.normalizeRadians((longitude - slongitude).toRadians) * SunMoonCalculator1.LUNAR_CYCLE_DAYS / (2 * Double.pi)
		...
		return [longitude, latitude, distance * SunMoonCalculator1.EARTH_RADIUS / SunMoonCalculator1.AU, atan(1737.4 / (distance * SunMoonCalculator1.EARTH_RADIUS))]
	}
	...
	private func doCalc(_ pos:[Double]) -> [Double] {
		...
		let jd0:Double = floor(jd_UT - 0.5) + 0.5,
				T0:Double = (jd0 - SunMoonCalculator1.J2000) / SunMoonCalculator1.JULIAN_DAYS_PER_CENTURY,
				secs:Double = (jd_UT - jd0) * SunMoonCalculator1.SECONDS_PER_DAY
		var gmst:Double = (((((-6.2e-6 * T0) + 9.3104e-2) * T0) + 8640184.812866) * T0) + 24110.54841
		let msday:Double = 1 + (((((-1.86e-5 * T0) + 0.186208) * T0) + 8640184.812866) / (SunMoonCalculator1.SECONDS_PER_DAY * SunMoonCalculator1.JULIAN_DAYS_PER_CENTURY))
		...
		let radiusAU:Double = SunMoonCalculator1.EARTH_RADIUS / SunMoonCalculator1.AU
		...
		let celestialHoursToEarthTime:Double = 180 / (15 * Double.pi) / 24 / SunMoonCalculator1.SIDEREAL_DAY_LENGTH

		// Make calculations for the meridian
		let transit_time1:Double = celestialHoursToEarthTime * SunMoonCalculator1.normalizeRadians(ra - lst),
				transit_time2:Double = celestialHoursToEarthTime * (SunMoonCalculator1.normalizeRadians(ra - lst) - 2 * Double.pi)
		...
		if abs(tmp) <= 1 {
			let ang_hor:Double = abs(acos(tmp)),
					rise_time1:Double = celestialHoursToEarthTime * SunMoonCalculator1.normalizeRadians(ra - ang_hor - lst),
					set_time1:Double = celestialHoursToEarthTime * SunMoonCalculator1.normalizeRadians(ra + ang_hor - lst),
					rise_time2:Double = celestialHoursToEarthTime * (SunMoonCalculator1.normalizeRadians(ra - ang_hor - lst) - 2 * Double.pi),
					set_time2:Double = celestialHoursToEarthTime * (SunMoonCalculator1.normalizeRadians(ra + ang_hor - lst) - 2 * Double.pi)
	...
	private func obtainAccurateRiseSetTransit(riseSetJD:Double, index:Int, niter:Int, sun:Bool) -> Double {
		...
		if step > 1 / SunMoonCalculator1.SECONDS_PER_DAY {
	...
	private func getMoonDiskOrientationAngles(lst:Double, sunRA:Double, sunDec:Double,
		...
		let W:Double = moonLon - omega,
				...
				lp:Double = SunMoonCalculator1.normalizeRadians(A - F),
	...
	var moonPhase: MoonPhase {
        return SunMoonCalculator1.getMoonPhaseName(lunarAge: moonAge) ?? .newMoon
	}
    /** Moon sign*/
    func moonSign(date: Date) -> MoonZodiacSign{
        return MoonZodiacSignCalculator().getMoonZodicaSign(date: date)
    }
}
