...
public final class SunMoonCalculator {
     ...
     */
    public func getMoonIllumination(date: Date) -> Illumination {
        ...
        let model = Illumination(fraction: (1.0 + cos(inc)) / 2.0, phase: 0.5 + 0.5 * inc * (angle < 0.0 ? -1.0 : 1.0) / .pi, angle: angle)
...
extension SunMoonCalculator {
    ...
    private func toDays(date: Date) -> Double {
        return toJulian(date: date) - SunMoonCalculator.J2000
    }
    
    private func toJulian(date: Date) -> Double {
        return Double(date.millisecondsSince1970) / SunMoonCalculator.dayMs - 0.5 + SunMoonCalculator.J1970
    }
    ...
    private func rightAscension(l: Double, b: Double) -> Double {
        return atan2(sin(l) * cos(SunMoonCalculator.e) - tan(b) * sin(SunMoonCalculator.e), cos(l))
    }
    
    private func declination(l: Double, b: Double) -> Double {
        return asin(sin(b) * cos(SunMoonCalculator.e) + cos(b) * sin(SunMoonCalculator.e) * sin(l))
    }
    ...
    private func setUTDate(_ jd: Double) {
        jd_UT = jd
        t = (jd + TTminusUT / SunMoonCalculator.SECONDS_PER_DAY - SunMoonCalculator.J2000) / SunMoonCalculator.JULIAN_DAYS_PER_CENTURY
    }
    ...
    private func getCurrentDate(date: Date, location: CLLocation) -> (y: Int, month: Int, d: Int, h: Int, m: Int, s: Int, lat: Double, lon: Double) {
        ...
        let lat = location.coordinate.latitude * SunMoonCalculator.DEG_TO_RAD
        let lon = location.coordinate.longitude * SunMoonCalculator.DEG_TO_RAD
