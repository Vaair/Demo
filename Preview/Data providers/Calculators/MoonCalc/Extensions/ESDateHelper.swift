...
public extension Date {
    ...
    func toString(format: DateFormatType, timeZone: TimeZoneType = .local, locale: Locale = Locale.init(identifier: "ru_RU")) -> String {
        switch format {
        case .dotNet:
    ...
    func toString(dateStyle: DateFormatter.Style, timeStyle: DateFormatter.Style, isRelative: Bool = false, timeZone: Foundation.TimeZone = Foundation.NSTimeZone.local, locale: Locale = Locale.init(identifier: "ru_RU")) -> String {
    ...
    func toStringWithRelativeTime(strings: [RelativeTimeStringType: String]? = nil) -> String {
        ...
        let sec: Int = Calendar.current.dateComponents([.second],
                                                       from: self,
                                                       to: Date()).second ?? 0//abs(now - time)
        let min: Int = Calendar.current.dateComponents([.minute],
                                                       from: self,
                                                       to: Date()).minute ?? 0//round(sec / 60)
        let hr: Int = Calendar.current.dateComponents([.hour],
                                                      from: self,
                                                      to: Date()).hour ?? 0//round(min / 60)
        let d: Int = Calendar.current.dateComponents([.day],
                                                     from: self,
                                                     to: Date()).day ?? 0// round(hr / 24)
        if min < 1 && hr < 1 && d < 1 {
            if sec < 10 {
                if isPast {
                    return strings?[.nowPast] ?? NSLocalizedString("Только что", comment: "Date format")
                } else {
                    return strings?[.nowFuture] ?? NSLocalizedString("через несколько секунд", comment: "Date format")
                }
            } else if sec > 20 && (sec % 10 == 2 || sec % 10 == 3 || sec % 10 == 4){
                let string: String
                if isPast {
                    string = strings?[.secondsPast] ?? NSLocalizedString("%.d секунды назад", comment: "Date format")
                } else {
                    string = strings?[.secondsFuture] ?? NSLocalizedString("в течении %.d секунд", comment: "Date format")
                }
                print("sec", sec)
                ...
                let string: String
                if isPast {
                    string = strings?[.secondsPast] ?? NSLocalizedString("%.d секунд назад", comment: "Date format")
                } else {
                    string = strings?[.secondsFuture] ?? NSLocalizedString("в течении %.d секунд", comment: "Date format")
                }
                print("sec", sec)
        ...
        if hr < 1 && d < 1 {
            if min == 1 {
                if isPast {
                    return strings?[.oneMinutePast] ?? NSLocalizedString("минуту назад", comment: "Date format")
                } else {
                    return strings?[.oneMinuteFuture] ?? NSLocalizedString("в течении %.d минуты", comment: "Date format")
                }
            } else if min > 20 && (min % 10 == 2 || min % 10 == 3 || min % 10 == 4){
                let string: String
                if isPast {
                    string = strings?[.secondsPast] ?? NSLocalizedString("%.d минуты назад", comment: "Date format")
                } else {
                    string = strings?[.secondsFuture] ?? NSLocalizedString("в течении %.d минут", comment: "Date format")
                }
                return String(format: string, min)
            } else {
                let string: String
                if isPast {
                    string = strings?[.minutesPast] ?? NSLocalizedString("%.d минут назад", comment: "Date format")
                } else {
                    string = strings?[.minutesFuture] ?? NSLocalizedString("в течении %.d минут", comment: "Date format")
                }
                return String(format: string, min)
            }
        }
        if d <= 1 {
            if hr == 1 {
                if isPast {
                    return strings?[.oneHourPast] ?? NSLocalizedString("час назад", comment: "Date format")
                } else {
                    return strings?[.oneHourFuture] ?? NSLocalizedString("в течении часа", comment: "Date format")
                }
            } else if hr < 10 && (hr == 2 || hr == 3 || hr == 4){
                let string: String
                if isPast {
                    string = strings?[.secondsPast] ?? NSLocalizedString("%.d часа назад", comment: "Date format")
                } else {
                    string = strings?[.secondsFuture] ?? NSLocalizedString("в течении %.d часов", comment: "Date format")
                }
                return String(format: string, hr)
            } else {
                let string: String
                if isPast {
                    string = strings?[.hoursPast] ?? NSLocalizedString("%.d часов назад", comment: "Date format")
                } else {
                    string = strings?[.hoursFuture] ?? NSLocalizedString("в течении %.d часов", comment: "Date format")
                }
                return String(format: string, hr)
            }
        }
        if d > 1 && d < 7 {
            if d == 1 {
                if isPast {
                    return strings?[.oneDayPast] ?? NSLocalizedString("вчера", comment: "Date format")
                } else {
                    return strings?[.oneDayFuture] ?? NSLocalizedString("завтра", comment: "Date format")
                }
            } else if (d == 2 || d == 3 || d == 4){
                let string: String
                if isPast {
                    string = strings?[.secondsPast] ?? NSLocalizedString("%.d дня назад", comment: "Date format")
                } else {
                    string = strings?[.secondsFuture] ?? NSLocalizedString("в течении %.d дней", comment: "Date format")
                }
                return String(format: string, d)
            } else {
                let string: String
                if isPast {
                    string = strings?[.daysPast] ?? NSLocalizedString("%.d дней назад", comment: "Date format")
                } else {
                    string = strings?[.daysFuture] ?? NSLocalizedString("в течении %.d дней", comment: "Date format")
                }
                return String(format: string, d)
            }
        }
        
        return self.toString(format: .custom("dd.MM.yyyy"))
    }
    ...
    private static func cachedFormatter(_ format: String = DateFormatType.standard.stringFormat, timeZone: Foundation.TimeZone = Foundation.TimeZone.current, locale: Locale = Locale.init(identifier: "ru_RU")) -> DateFormatter {
    ...
    private static func cachedFormatter(_ dateStyle: DateFormatter.Style, timeStyle: DateFormatter.Style, doesRelativeDateFormatting: Bool, timeZone: Foundation.TimeZone = Foundation.NSTimeZone.local, locale: Locale = Locale.init(identifier: "ru_RU")) -> DateFormatter {
